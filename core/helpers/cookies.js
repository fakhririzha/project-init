...
const set = (key, value) => {
    ...
    if (str && str.length > maxChar) {
        ...
        for (let i = 0; i < str.length; i += maxChar) {
            Cookies.set(`__${key}_${i}`, str.slice(i, i + maxChar), {
                expires: expiredDefault,
            });
            subKeys += i + maxChar >= str.length ? i : `${i},`;
            if (i + maxChar >= str.length) {
                Cookies.set(`__${key}_subKeys`, JSON.stringify(subKeys), {
                    expires: expiredDefault,
                });
            }
        }
    ...
    try {
        ...
        if (subKeys) {
            const str = subKeys
                ...
                .reduce(
                    (result, subKey) =>
                        result + Cookies.getJSON(`__${key}_${subKey}`),
                    ''
                );
...
const remove = (key) => {
    ...
    if (subKeys) {
        subKeys
            .split(',')
            .forEach((subKey) => Cookies.remove(`__${key}_${subKey}`));
